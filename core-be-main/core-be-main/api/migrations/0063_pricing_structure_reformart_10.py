...
def two_part_formula_minimum_commit(metric, lookup_value):
    for item in rows:
        if item[metric]["low"] <= lookup_value <= item[metric]["high"]:
            return (item["transaction_fee"] * item["transaction_fee"]) + (
                lookup_value * item["platform_fee"]
            )
...
def migrate_pricing_structure_format10(apps, schema_editor):
...
class Migration(migrations.Migration):
    dependencies = [
        ("api", "0062_contractsignerdetails_signer_type"),
    ]

    operations = [migrations.RunPython(migrate_pricing_structure_format10)]
